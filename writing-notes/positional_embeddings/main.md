...
Suppose you have a of messy, unorganized text inputs that you need to sort out. Think of it like having a massive basket filled with a chaotic mix of fruits. Ideally, you want to separate them so that similar fruits end up together.
...
Equivalently, $PE(p)$ can be written as alternating sine and cosine pairs at increasing frequencies:
...
If $\left\lVert u \right\lVert$ and $\left\lVert v \right\lVert$ are **constant**, then the distance between $u$ and $v$ is determined entirely by their inner product $\langle u, v \rangle$.  Later on this article, we show that the norm for each positional vector after postional encoding is a constant. 
...
From Section 4, $\left\lVert PE(p_1)-PE(p_2) \right\lVert^2 \in [0, 2d]$. Dividing by $2d$:
