...
Let us mention some remarks on senusoidal positional encoding. We satrt we simple dot product concept. 
...
Given a vector $v \in \mathbb{R}^d$ (for any $d \in \mathbb{R}^d$), its **norm** (length) is defined from the inner product with itself:
...
$$\omega_i = \frac{1}{\omega^{2i/d}}, \qquad i = 0, 1, \dots, d-1$$
